...
def top_metrics(y_true: Sequence[float], y_pred: Sequence[float], k: int) -> Dict[str, float]:
    ...
    return {
        "ndcg_at_k": dcg([rel[i] for i in pred_idx]) / denom if denom else 0.0,
        "top1pct_recall_at_k": len(pred_set & top1) / len(top1),
        "best_true_in_pred_top_k": max(y_true[i] for i in pred_idx),
    }


def solve_linear(features: Sequence[Sequence[float]], targets: Sequence[float], ridge: float = 1e-8) -> Optional[List[float]]:
    if not features:
        return None
    p = len(features[0])
...
def eval_prediction(
    ...
    return {
        ...
        "n_test": len(y_all),
        "spearman_all": "" if rho_all is None else rho_all,
        "n_test_doubles": len(y_double),
        "spearman_doubles": "" if rho_double is None else rho_double,
        "n_test_higher": len(y_higher),
        "spearman_higher": "" if rho_higher is None else rho_higher,
        "ndcg_at_100": metrics["ndcg_at_k"],
        "top1pct_recall_at_100": metrics["top1pct_recall_at_k"],
        "best_true_in_pred_top_100": metrics["best_true_in_pred_top_k"],
    }


def process_assay(
    assay: str,
    assay_groups: Sequence[Tuple[Tuple[str, int, int, str], Sequence[str]]],
    ...
    for key, train_mutants in assay_groups:
        ...
        for method in methods:
            ...
            if w1 is not None:
                pred = {
                    mutant: dot(w1, [1.0, record[method]])
                    for mutant, record in table.items()
                    if method in record
                }
            ...
            if w2 is not None:
                pred = {
                    mutant: dot(w2, [1.0, add_pred[mutant], record[method]])
                    for mutant, record in table.items()
                    if method in record
                }

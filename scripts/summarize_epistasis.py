...
def summarize_residuals(residuals: Path, out: Path) -> None:
    ...
    for assay, values in by_assay.items():
        rows.append(
            {
                "assay": assay,
                "n_strict_doubles": len(values),
                "mean_abs_epistasis_residual": mean(values),
                "median_abs_epistasis_residual": median(values),
                "sign_epistasis_candidate_count": sign_counts[assay],
                "sign_epistasis_candidate_fraction": sign_counts[assay] / len(values),
            }
        )
            ...
            by_method[row["method"]].append(
                {
                    ...
                    "abs_rho": rho,
                    "rho_abs_res": rho_abs_res if rho_abs_res is not None else float("nan"),
                    "high_recall": high_recall if high_recall is not None else float("nan"),
                }
            )
    rows = []
        ...
        rows.append(
            {
                ...
                "mean_abs_spearman_double_score": mean(r["abs_rho"] for r in records),
                "mean_gain_vs_additive_abs_spearman": mean(r["gain"] for r in records),
                "assays_positive_gain": sum(1 for r in records if r["gain"] > 0),
                "mean_spearman_abs_epistasis_residual": mean(
                    r["rho_abs_res"] for r in records if r["rho_abs_res"] == r["rho_abs_res"]
                ),
                "mean_top100_high_abs_residual_recall": mean(
                    r["high_recall"] for r in records if r["high_recall"] == r["high_recall"]
                ),
            }
        )

...
def compare(
    ...
    family: str,
    metric: str,
    data: Dict[str, Dict[Tuple[str, ...], float]],
    method_a: str,
    method_b: str,
) -> None:
    a = data.get(method_a, {})
    ...
    rows.append(
        {
            ...
            "mean_diff_a_minus_b": mean(diffs),
...
def main() -> None:
    rows: List[Dict[str, object]] = []
    zero = load_metric_map(
        ROOT / "results" / "zero_shot" / "zero_shot_all_methods_metrics.csv",
        ["assay"],
        "method",
        "abs_spearman",
    )
    for a, b in [
        ("VenusREM", "ProSST-2048"),
        ("ProSST-2048", "ESM3"),
        ("S3F_MSA", "ESM3"),
        ("ProSST-2048", "Site_Independent"),
    ]:
        compare(rows, "zero_shot", "abs_spearman", zero, a, b)

    higher = load_metric_map(
        ROOT / "results" / "single_to_multi" / "single_to_multi_representative_metrics.csv",
        ["assay"],
        "method",
        "abs_spearman",
        {"order_bucket": "higher"},
    ...
    for a, b in [
        ("MSA_Transformer_ensemble", "additive_all_singles"),
        ("GEMME", "additive_all_singles"),
        ("ESM3", "additive_all_singles"),
        ("ProSST-2048", "additive_all_singles"),
    ]:
        compare(rows, "single_to_higher", "abs_spearman", higher, a, b)

    active = load_metric_map(
        ROOT / "results" / "active_learning" / "active_learning_representative_metrics.csv",
        ["assay", "seed"],
        "policy",
        "top1pct_found",
        {"round": "5"},
    ...
    for a, b in [
        ("diverse_ensemble:top_methods", "random"),
        ("diverse_ensemble:top_methods", "additive_lookup"),
        ("ensemble_ucb:top_methods", "zscore_linear:ESM3"),
        ("ensemble_mean:top_methods", "zscore_linear:ESM3"),
    ]:
        compare(rows, "active_learning_round5", "top1pct_found", active, a, b)

    pairwise = load_metric_map(
        ROOT / "results" / "multi_evolve_style" / "pairwise_plm_residual_higher_order_metrics.csv",
        ["assay", "seed"],
        "model",
        "abs_spearman_higher",
        {"selection_policy": "random_doubles", "double_budget": "100"},
    )
    for a, b in [
        ("pairwise_plus_zscore:S2F_MSA", "pairwise_residual_from_doubles"),
        ("pairwise_plus_plm_ensemble", "pairwise_residual_from_doubles"),
        ("pairwise_plus_zscore:ProSST-2048", "pairwise_residual_from_doubles"),
    ]:

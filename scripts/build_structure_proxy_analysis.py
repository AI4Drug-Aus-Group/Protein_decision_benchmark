...
STRUCTURE_DIRS = [
    ROOT / "proteingym" / "structures" / "AF2",
    ROOT / "proteingym" / "structures",
]
...
def assay_type_map() -> Dict[str, str]:
    ...
    with path.open(newline="", encoding="utf-8") as fh:
        return {
            r["assay"]: r.get("assay_type_official", r.get("assay_type_refined", r.get("assay_type_curated", r.get("assay_type", "uncurated"))))
...
def main() -> None:
            ...
            proxy_rows.append(
                {
                    ...
                    "assay_type": types.get(row["assay"], "uncurated"),
                    "has_pdb_in_official_reference": int(bool(pdb_file)),
                    "pdb_file": pdb_file,
                    "pdb_range": meta.get("pdb_range", ""),
                    "has_local_structure_file": int(bool(pdb_path)),
                    "local_structure_path": pdb_path,
                }
            )
    ...
    for (atype, has_pdb, has_local), gains in groups.items():
        summary.append(
            {
                "assay_type": atype,
                "has_pdb_in_official_reference": has_pdb,
                "has_local_structure_file": has_local,
                "n_assays": len(gains),
                "mean_structure_aware_gain": mean(gains),
            }
        )

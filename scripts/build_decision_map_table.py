...
def read_assay_types() -> Dict[str, str]:
    ...
    with path.open(newline="", encoding="utf-8") as fh:
        return {
            r["assay"]: r.get(
                "assay_type_official",
                r.get("assay_type_refined", r.get("assay_type", r.get("assay_type_curated", "uncurated"))),
            )
...
def low_n_gain() -> Dict[str, Dict[str, object]]:
    sources = [
        (
            ROOT / "results" / "low_n" / "low_n_calibration_top20_nongiant_metrics.percentile.csv",
            "full candidate pool",
        ),
        (
            ROOT / "results" / "low_n" / "low_n_calibration_top20_giant_fast_metrics.percentile.csv",
            "deterministic 20,000-candidate sample",
        ),
    ]
    values: Dict[Tuple[str, str], List[float]] = defaultdict(list)
    ...
    for assay, rec in out.items():
        ...
        best_plm = max(
            (
                ...
                if k.startswith("low_n20_mixed_best_pct_")
                and k != "low_n20_mixed_best_pct_additive_lookup"
            ),
            default=None,
        )
...
def main() -> None:
    ...
    for assay, row in manifest.items():
        rec: Dict[str, object] = {
            ...
            "assay_type": assay_types.get(assay, "uncurated"),
            "n_variants": row["n_variants"],
            "n_singles": row["n_singles"],
            "n_doubles": row["n_doubles"],
            "n_higher_order": row["n_higher_order"],
            "strict_double_evaluable": row["strict_double_evaluable"],
            "strict_higher_evaluable": row["strict_higher_evaluable"],

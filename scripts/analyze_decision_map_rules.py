...
def strategy(row: dict[str, str], epi_low: float, epi_high: float, neff_low: float, neff_high: float) -> str:
    ...
    if struct_gain is not None and struct_gain >= 0.10:
        ...
        out.append(
            {
                ...
                "bin": group,
                "n_assays": len(rs),
                "mean_low_n_plm_gain_over_additive": mean(plm),
                "mean_active_gain_diverse_ensemble_vs_random": mean(active),
                "mean_epistasis_abs_residual": mean(epi),
                "mean_prosst_gain_over_site_independent": mean(struct),
                "recommended_default": max(
                    defaultdict(int, {s: sum(1 for r in rs if r["recommended_strategy"] == s) for s in {r["recommended_strategy"] for r in rs}}),
                    key=lambda s: sum(1 for r in rs if r["recommended_strategy"] == s),
                ),
            }
        )
...
def main() -> None:
    ...
    for row in rows:
        rec = {
            ...
            "best_zero_shot_method": row.get("best_zero_shot_method", ""),
            "recommended_strategy": strategy(row, epi_low, epi_high, neff_low, neff_high),
            "epistasis_bin": bin_value(parse_float(row.get("epistasis_mean_abs_epistasis_residual", "")), epi_low, epi_high, ("low", "mid", "high")),
            "official_msa_neff_l_bin": bin_value(parse_float(row.get("official_MSA_Neff_L", "")), neff_low, neff_high, ("low", "mid", "high")),
            "low_n20_best_plm_gain_over_additive": row.get("low_n20_best_plm_gain_over_additive", ""),
            "active_gain_diverse_ensemble_vs_random": row.get("active_gain_diverse_ensemble_vs_random", ""),
            "zero_abs_spearman_ProSST-2048": row.get("zero_abs_spearman_ProSST-2048", ""),
            "zero_abs_spearman_Site_Independent": row.get("zero_abs_spearman_Site_Independent", ""),
        }
        recs.append(rec)

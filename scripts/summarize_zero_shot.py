...
def summarize(metrics_path: Path, out_dir: Path) -> None:
    ...
    with metrics_path.open(newline="", encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            ...
            record = {
                ...
                "ndcg_at_k": to_float(row.get("ndcg_at_k", "")) or 0.0,
                "top1pct_recall_at_k": to_float(row.get("top1pct_recall_at_k", "")) or 0.0,
                "top5pct_recall_at_k": to_float(row.get("top5pct_recall_at_k", "")) or 0.0,
            }
    ...
    for method, records in by_method.items():
        method_rows.append(
            {
                "method": method,
                "category": categories.get(method, "unknown"),
                "n_assays": len(records),
                "mean_spearman": mean(r["spearman"] for r in records),
                "median_spearman": median(r["spearman"] for r in records),
                "mean_abs_spearman": mean(r["abs_spearman"] for r in records),
                "median_abs_spearman": median(r["abs_spearman"] for r in records),
                "mean_ndcg_at_100": mean(r["ndcg_at_k"] for r in records),
                "mean_top1pct_recall_at_100": mean(r["top1pct_recall_at_k"] for r in records),
                "mean_top5pct_recall_at_100": mean(r["top5pct_recall_at_k"] for r in records),
                "best_abs_spearman_assay_wins": sum(
                    1 for best_method, _ in by_assay_best.values() if best_method == method
                ),
            }
        )
        ...
        category_rows.append(
            {
                "category": category,
                "n_methods": len(rows),
                "mean_of_method_mean_abs_spearman": mean(float(row["mean_abs_spearman"]) for row in rows),
                "best_method": rows[0]["method"],
                "best_method_mean_abs_spearman": rows[0]["mean_abs_spearman"],
                "assay_wins": sum(int(row["best_abs_spearman_assay_wins"]) for row in rows),
            }
        )
    ...
    for row in method_rows[:15]:
        print(
            row["method"],
            row["category"],
            f"{float(row['mean_abs_spearman']):.4f}",
            f"wins={row['best_abs_spearman_assay_wins']}",
        )

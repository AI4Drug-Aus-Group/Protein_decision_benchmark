            ...
            out.append(
                {
                    ...
                    "k": k,
                    "n_variants": len(rows),
                    "high_abs_residual_rate": abs_rate,
                    "high_abs_residual_enrichment": abs_rate / base_abs if base_abs else float("nan"),
                    "positive_residual_rate": pos_rate,
                    "positive_residual_enrichment": pos_rate / base_pos if base_pos and not math.isnan(base_pos) else float("nan"),
                    "sign_epistasis_rate": sign_rate,
                    "sign_epistasis_enrichment": sign_rate / base_sign if base_sign else float("nan"),
                }
            )
    return out
...
def main() -> None:
    ...
    for r in resid_rows:
        ...
        score_rec = {
            "assay": r["assay"],
            "mutant": r["mutant"],
            "epistasis_residual": float(r["epistasis_residual"]),
            "abs_epistasis_residual": float(r["abs_epistasis_residual"]),
            "sign_epistasis_candidate": float(r["sign_epistasis_candidate"]),
            "ensemble_mean": ensemble_mean,
            "ensemble_std": stdev(vals),
            "additive_ensemble_disagreement": abs((additive or 0.0) - ensemble_mean),
            "sequence_structure_disagreement": abs(mean(seq_vals) - mean(struct_vals)) if seq_vals and struct_vals else 0.0,
        }
        ...
        summary.append(
            {
                ...
                "k": k,
                "n_assays": len(rows),
                "mean_high_abs_residual_enrichment": mean([float(r["high_abs_residual_enrichment"]) for r in rows if not math.isnan(float(r["high_abs_residual_enrichment"]))]),
                "mean_positive_residual_enrichment": mean([float(r["positive_residual_enrichment"]) for r in rows if not math.isnan(float(r["positive_residual_enrichment"]))]),
                "mean_sign_epistasis_enrichment": mean([float(r["sign_epistasis_enrichment"]) for r in rows if not math.isnan(float(r["sign_epistasis_enrichment"]))]),
            }
        )

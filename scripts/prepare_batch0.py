...
def build_manifest() -> Dict[str, object]:
    ...
    for path in assay_files():
        ...
        strict_double = sum(
            1 for _, order, parts in variants if order == 2 and all(part in singles for part in parts)
        )
        strict_higher = sum(
            1 for _, order, parts in variants if order >= 3 and all(part in singles for part in parts)
        )
        stem = path.stem
        ...
        row = {
            "assay": path.name,
            "protein_key": protein_key(stem),
            "n_variants": len(variants),
            "n_singles": order_counts.get(1, 0),
            "n_doubles": order_counts.get(2, 0),
            "n_higher_order": sum(count for order, count in order_counts.items() if order >= 3),
            "max_order": max(order_counts) if order_counts else 0,
            "strict_double_evaluable": strict_double,
            "strict_higher_evaluable": strict_higher,
            "has_msa": int(has_msa_for_assay(stem, msa_files)),
            "has_zero_shot": int(path.name in zero_csvs),
            "has_duplicate_top_level_csv": int((TOP_EXTRACTED / path.name).exists()),
    ...
    fieldnames = [
        ...
        "n_higher_order",
        "max_order",
        "strict_double_evaluable",
        "strict_higher_evaluable",
        "has_msa",
        "has_zero_shot",
        "has_duplicate_top_level_csv",
        "columns",
    ] + [f"n_order_{order}" for order in range(1, 11)] + ["n_order_gt10"]
    ...
    summary = {
        ...
        "dms_dir": str(DMS_DIR.relative_to(ROOT)),
        "duplicate_top_level_csv_count": top_level_duplicates,
        "msa_files": len(msa_files),
        "zero_shot_zip_exists": ZERO_ZIP.exists(),
        "zero_shot_csv_count": len(zero_csvs),
        "assays_with_msa": sum(row["has_msa"] for row in rows),
        "assays_with_zero_shot": sum(row["has_zero_shot"] for row in rows),
        "assays_with_doubles": sum(1 for row in rows if row["n_doubles"] > 0),
        "assays_with_higher_order": sum(1 for row in rows if row["n_higher_order"] > 0),
        "assays_with_100plus_strict_doubles": sum(1 for row in rows if row["strict_double_evaluable"] >= 100),
        "assays_with_100plus_strict_higher": sum(1 for row in rows if row["strict_higher_evaluable"] >= 100),
        "mutation_order_counts": {str(k): v for k, v in sorted(total_order_counts.items())},
    }
...
def split_for_assay(path: Path, budgets: Sequence[int], seeds: Sequence[int]) -> List[Dict[str, object]]:
    ...
    for seed in seeds:
        for budget in budgets:
            ...
            regimes["single_only"] = sample_without_replacement(
                singles, budget, random.Random(f"{path.name}|{seed}|{budget}|single_only")
            )
            regimes["mixed_random"] = sample_without_replacement(
                all_variants, budget, random.Random(f"{path.name}|{seed}|{budget}|mixed_random")
            )
            ...
            if len(selected) < budget:
                selected.extend(
                    mutant
                    for mutant in sample_without_replacement(all_variants, budget, rng)
                    if mutant not in set(selected)
                )

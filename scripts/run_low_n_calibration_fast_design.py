...
def additive_predictions(
    table: Dict[str, Dict[str, float]], train_set: set[str], mutants: Sequence[str]
) -> Dict[str, float]:
...
def evaluate(
    ...
    return {
        ...
        "n_test": len(sample),
        "spearman_all": "" if rho is None else rho,
        "spearman_sampled": 1,
        "spearman_sample_size": len(sample),
        "design_sampled": 1,
        "design_sample_size": len(sample),
        "n_test_doubles": "",
        "spearman_doubles": "",
        "n_test_higher": "",
        "spearman_higher": "",
        "ndcg_at_100": "",
        "top1pct_recall_at_100": len(pred_set & top_true) / len(top_true),
...
def process_assay(
    ...
    for key, train_mutants in assay_groups:
        ...
        for method in methods:
            ...
            if w1 is not None:
                pred = {
                    m: dot(w1, [1.0, table[m][method]])
                    for m in candidate_sample
                    if m in table and method in table[m]
                }
                rows.append(evaluate(assay, key, "zscore_linear", method, table, train_set, pred, sample_size, k))
            w2 = solve_linear(f2, train_y)
            if w2 is not None:
                pred = {
                    m: dot(w2, [1.0, add_pred[m], table[m][method]])
                    for m in candidate_sample
                    if m in add_pred and m in table and method in table[m]
                }
...
def main() -> None:
    ...
    with out_path.open("w", newline="", encoding="utf-8") as fh:
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as pool:
            futures = [
                pool.submit(process_assay, assay, assay_groups, methods, args.sample_size, args.k)
                for assay, assay_groups in groups.items()
            ]

...
def main() -> None:
    ...
    for assay, rows in by_assay.items():
        ...
        positive_hits = {m for v, m in positive_res[: max(1, math.ceil(len(positive_res) * 0.10))] if v > 0}
        sign_hits = {
            r["mutant"]
            for r in rows
            if (r.get("sign_epistasis_candidate") or r.get("sign_epistasis") or r.get("is_sign_epistasis") or "") in {"1", "True", "true"}
        }
        if not sign_hits:
            sign_hits = high_abs
                ...
                detail.append(
                    {
                        ...
                        "k": k,
                        "n_doubles": len(rows),
                        "high_abs_residual_hit_rate": high_rate,
                        "high_abs_residual_enrichment": high_rate / expected_high if expected_high else "",
                        "positive_residual_hit_rate": positive_rate,
                        "positive_residual_enrichment": positive_rate / expected_positive if expected_positive else "",
                        "sign_or_high_residual_hit_rate": sign_rate,
                        "sign_or_high_residual_enrichment": sign_rate / expected_sign if expected_sign else "",
                    }
                )
        ...
        summary.append(
            {
                ...
                "k": k,
                "n_assays": len(rows),
                "mean_high_abs_residual_hit_rate": mean(float(r["high_abs_residual_hit_rate"]) for r in rows),
                "mean_high_abs_residual_enrichment": mean(float(r["high_abs_residual_enrichment"]) for r in rows if r["high_abs_residual_enrichment"] != ""),
                "mean_positive_residual_hit_rate": mean(float(r["positive_residual_hit_rate"]) for r in rows),
                "mean_positive_residual_enrichment": mean(float(r["positive_residual_enrichment"]) for r in rows if r["positive_residual_enrichment"] != ""),
                "mean_sign_or_high_residual_hit_rate": mean(float(r["sign_or_high_residual_hit_rate"]) for r in rows),
                "mean_sign_or_high_residual_enrichment": mean(float(r["sign_or_high_residual_enrichment"]) for r in rows if r["sign_or_high_residual_enrichment"] != ""),
            }
        )

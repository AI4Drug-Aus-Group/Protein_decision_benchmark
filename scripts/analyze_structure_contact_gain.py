    ...
    with GAIN.open(newline="", encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            ...
            rows.append(
                {
                    **row,
                    "n_double_variants": c.get("n_double_variants", ""),
                    "n_coord_complete_double": c.get("n_coord_complete_double", ""),
                    "coord_complete_double_fraction": c.get("coord_complete_double_fraction", ""),
                    "double_mean_min_ca_distance": c.get("double_mean_min_ca_distance", ""),
                    "double_fraction_min_ca_lt8": c.get("double_fraction_min_ca_lt8", ""),
                    "double_fraction_min_ca_lt12": c.get("double_fraction_min_ca_lt12", ""),
                    "double_close_bin": bin_close_fraction(frac),
                }
            )
    ...
    out = [
        {
            ...
            "double_close_bin": close_bin,
            "n_assays": len(vals),
            "mean_structure_aware_gain": mean(vals),
        }
        for (assay_type, close_bin), vals in groups.items()
    ]

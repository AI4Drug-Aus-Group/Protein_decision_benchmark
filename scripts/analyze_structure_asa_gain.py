    ...
    with GAIN.open(newline="", encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            ...
            rows.append(
                {
                    **row,
                    "mean_neighbor_count_10A": a.get("mean_neighbor_count_10A", ""),
                    "mean_exposed_fraction": a.get("mean_exposed_fraction", ""),
                    "mean_buried_fraction": a.get("mean_buried_fraction", ""),
                    "multi_exposed_fraction": a.get("multi_exposed_fraction", ""),
                    "multi_buried_fraction": a.get("multi_buried_fraction", ""),
                    "exposure_bin": exposure_bin(exposed, buried),
                }
            )
    ...
    out = [
        {
            ...
            "exposure_bin": bin_name,
            "n_assays": len(vals),
            "mean_structure_aware_gain": mean(vals),
        }
        for (assay_type, bin_name), vals in groups.items()
    ]

        ...
        groups[policy].append(
            {
                ...
                "first_hit_round": float(hit[0]) if hit else float("nan"),
                "labels_to_first_hit": float(hit[1]) if hit else float("nan"),
                "censored_labels": float(final_n),
            }
        )
        ...
        rows.append(
            {
                "policy": policy,
                "n_runs": len(records),
                "hit_rate": mean(r["hit"] for r in records),
                "mean_first_hit_round_among_hits": mean(r["first_hit_round"] for r in records if r["first_hit_round"] == r["first_hit_round"]),
                "mean_labels_to_first_hit_among_hits": mean(r["labels_to_first_hit"] for r in records if r["labels_to_first_hit"] == r["labels_to_first_hit"]),
                "mean_censored_labels_all_runs": mean(r["labels_to_first_hit"] if r["labels_to_first_hit"] == r["labels_to_first_hit"] else r["censored_labels"] for r in records),
            }
        )

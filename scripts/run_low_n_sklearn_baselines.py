...
def top_metrics(y_true: Sequence[float], y_pred: Sequence[float], k: int) -> Dict[str, float]:
    ...
    return {
        "ndcg_at_100": dcg([rel[i] for i in pred_idx]) / denom if denom else 0.0,
        "top1pct_recall_at_100": len(set(pred_idx) & true_top1) / len(true_top1),
        "best_true_in_pred_top_100": max(y_true[i] for i in pred_idx),
    }
...
def model_factory(name: str, seed: int):
    ...
    if name == "gaussian_process":
        ...
        return make_pipeline(StandardScaler(), GaussianProcessRegressor(kernel=kernel, alpha=1e-6, normalize_y=True, random_state=seed, optimizer=None))
...
def eval_prediction(assay: str, key: Tuple[str, int, int, str], model: str, features_name: str, table: Dict[str, Dict[str, float]], train_set: set[str], pred: Dict[str, float], k: int) -> Dict[str, object]:
    ...
    return {
        ...
        "regime": regime,
        "model": model,
        "zero_shot_method": features_name,
        ...
        "top1pct_recall_at_100": metrics["top1pct_recall_at_100"],
        "best_true_in_pred_top_100": metrics["best_true_in_pred_top_100"],
    }

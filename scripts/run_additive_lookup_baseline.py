...
def evaluate_group_with_scores(
    key: Tuple[str, int, int, str], train_mutants: Sequence[str], scores: Dict[str, float]
) -> Dict[str, object]:
    ...
    return {
        ...
        "budget": budget,
        "regime": regime,
        "n_train": len(train_mutants),
        "n_train_singles": len(train_singles),
        "n_test": len(y_all),
...
def evaluate_assay_groups(
    assay: str, assay_groups: Sequence[Tuple[Tuple[str, int, int, str], Sequence[str]]]

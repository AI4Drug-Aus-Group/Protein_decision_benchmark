            ...
            by_method[row["method"]].append(
                {
                    ...
                    "rho_res": rho_res if rho_res is not None else float("nan"),
                }
            )
    rows = []
        ...
        rows.append(
            {
                ...
                "mean_abs_spearman_score": mean(r["abs_rho"] for r in records),
                "mean_top1pct_recall_at_100": mean(r["recall"] for r in records if r["recall"] == r["recall"]),
                "mean_spearman_residual": mean(r["rho_res"] for r in records if r["rho_res"] == r["rho_res"]),
            }
        )

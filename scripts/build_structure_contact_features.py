...
def summarize_distances(distances: List[float]) -> Dict[str, object]:
    if not distances:
        return {
            "n_coord_complete_multi": 0,
            "mean_min_ca_distance": "",
            "median_min_ca_distance": "",
            "fraction_min_ca_lt8": "",
            "fraction_min_ca_lt12": "",
            "fraction_min_ca_lt20": "",
        }
    return {
        "n_coord_complete_multi": len(distances),
        "mean_min_ca_distance": mean(distances),
        "median_min_ca_distance": median(distances),
        "fraction_min_ca_lt8": mean(1.0 if d < 8 else 0.0 for d in distances),
        "fraction_min_ca_lt12": mean(1.0 if d < 12 else 0.0 for d in distances),
        "fraction_min_ca_lt20": mean(1.0 if d < 20 else 0.0 for d in distances),
    }
...
def main() -> None:
    ...
    for meta in meta_rows:
        ...
        base = {
            ...
            "pdb_file": pdb_file,
            "pdb_range": meta.get("pdb_range", ""),
            "has_local_structure_file": int(pdb_path is not None),
            "n_multi_variants": 0,
            "n_coord_complete_multi": 0,
            "coord_complete_multi_fraction": "",
            "n_double_variants": 0,
            "n_coord_complete_double": 0,
            "coord_complete_double_fraction": "",
        }
        ...
        coords = align_coords(
            parse_ca_coords(pdb_path), target_positions, meta.get("pdb_range", "")
        )

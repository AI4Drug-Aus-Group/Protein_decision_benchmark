    ...
    for path in paths:
        ...
        with path.open(newline="", encoding="utf-8") as fh:
            for row in csv.DictReader(fh):
                ...
                groups[key].append(
                    {
                        "spearman_all": rho if rho is not None else float("nan"),
                        "abs_spearman_all": abs(rho) if rho is not None else float("nan"),
                        "top1pct_recall_at_100": top1 if top1 is not None else float("nan"),
                        "best_true_in_pred_top_100": best if best is not None else float("nan"),
                    }
                )
        ...
        rows.append(
            {
                ...
                "n_rows": len(records),
                "mean_abs_spearman_all": mean(
                    r["abs_spearman_all"] for r in records if r["abs_spearman_all"] == r["abs_spearman_all"]
                ),
                "mean_top1pct_recall_at_100": mean(
                    r["top1pct_recall_at_100"]
                    for r in records
                    if r["top1pct_recall_at_100"] == r["top1pct_recall_at_100"]
                ),
                "mean_best_true_in_pred_top_100": mean(
                    r["best_true_in_pred_top_100"]
                    for r in records
                    if r["best_true_in_pred_top_100"] == r["best_true_in_pred_top_100"]
                ),
            }
        )
    rows.sort(
        key=lambda row: (
            int(row["budget"]),
            row["regime"],
            -float(row["mean_top1pct_recall_at_100"]),
            -float(row["mean_abs_spearman_all"]),
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--inputs",
        default="results/low_n/low_n_calibration_representative_metrics.csv,results/low_n/low_n_calibration_giant_fast_metrics.csv",
    )

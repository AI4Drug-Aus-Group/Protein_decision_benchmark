...
def evaluate(assay: str, rows: list[dict[str, float]], budget: int, seed: int, policy: str) -> dict[str, object] | None:
    ...
    return {
        ...
        "epistasis_bin": residual_bin(rows),
        "n_train": len(train),
        "n_test": len(test),
        "spearman_additive": spearman(pred_add, true),
        "spearman_pairwise": spearman(pred_pair, true),
        "delta_spearman": spearman(pred_pair, true) - spearman(pred_add, true),
        "top1pct_recall_additive": top1_recall(pred_add, true),
        "top1pct_recall_pairwise": top1_recall(pred_pair, true),
        "delta_top1pct_recall": top1_recall(pred_pair, true) - top1_recall(pred_add, true),
    }
...
def main() -> None:
    ...
    with RESID.open(newline="", encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            by_assay[row["assay"]].append(
        {
            "mutant": row["mutant"],
                    "score_double": parse_float(row["score_double"]),
                    "additive_prediction": parse_float(row["additive_prediction"]),
                    "epistasis_residual": parse_float(row["epistasis_residual"]),
                    "abs_epistasis_residual": parse_float(row["abs_epistasis_residual"]),
                }
            )
    detail = []
        ...
        summary.append(
            {
                "budget": budget,
                "selection_policy": policy,
                "epistasis_bin": epi_bin,
                "n_runs": len(rows),
                "mean_delta_spearman": mean([float(r["delta_spearman"]) for r in rows if not math.isnan(float(r["delta_spearman"]))]),
                "mean_delta_top1pct_recall": mean([float(r["delta_top1pct_recall"]) for r in rows if not math.isnan(float(r["delta_top1pct_recall"]))]),
                "mean_pairwise_spearman": mean([float(r["spearman_pairwise"]) for r in rows if not math.isnan(float(r["spearman_pairwise"]))]),
            }
        )

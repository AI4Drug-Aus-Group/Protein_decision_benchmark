...
RULES = [
    ...
    ("expression_or_abundance", ["expression", "abundance", "surface", "high-expression", "low-expression", "hek293t", "rpe1"]),
    ("activity", ["activity", "casp", "dyr_", "blat_", "amie_", "esta_", "lgk_", "hmdh_", "hxk4", "cp2c9", "vkor1"]),
    ("viral_fitness_or_escape", ["hiv", "hv1", "sars", "spike", "zikv", "9infa", "i34a1", "i33a0", "aav", "polg_hcv", "polg_den", "rdrp", "ncap"]),
    ("drug_or_selection", ["nutlin", "etoposide", "das_25um", "h2o2", "antibiotic"]),
    ("stability_or_folding", ["tsuboyama_2023"]),
    ("function", ["function", "lof", "gof", "positive", "growth"]),
]
    ...
    with IN.open(newline="", encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            ...
            rows.append(
                {
                    **row,
                    "assay_type_refined": label,
                    "refined_confidence": confidence,
                    "refined_basis": basis,
                    "refined_keyword": keyword,
                    "manual_review_needed": int(confidence != "high"),
                }
            )

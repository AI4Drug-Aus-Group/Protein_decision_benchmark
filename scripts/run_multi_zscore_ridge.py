...
def process_assay(
    assay: str,
    groups: Sequence[Tuple[Tuple[str, int, int, str], Sequence[str]]],
        ...
        rows.append(
            {
                ...
                "best_true_in_pred_top_100": max(
                    y[i] for i in sorted(range(len(pred)), key=lambda j: pred[j], reverse=True)[: min(100, len(pred))]
                )
                if y
                else "",
            }
...
def main() -> None:
    ...
    with out.open("w", newline="", encoding="utf-8") as fh:
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as pool:
            futures = [
                pool.submit(process_assay, assay, assay_groups, feature_names, args.ridge)

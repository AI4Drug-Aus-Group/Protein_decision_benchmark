...
def top_metrics(y_true: Sequence[float], y_pred: Sequence[float], k: int) -> Dict[str, float]:
    ...
    return {
        "k": float(k),
        "ndcg_at_k": ndcg_num / ndcg_den if ndcg_den else 0.0,
        "top1pct_recall_at_k": len(pred_set & actual_top_1) / len(actual_top_1),
        "top5pct_recall_at_k": len(pred_set & actual_top_5) / len(actual_top_5),
        "best_true_in_pred_top_k": max(y_true[i] for i in top_pred_idx),
        "mean_true_in_pred_top_k": sum(y_true[i] for i in top_pred_idx) / k,
    }
...
def process_assay(assay: str, methods: Sequence[str], k: int, from_zip: bool) -> List[Dict[str, object]]:
    ...
    for method, (ys, preds) in y_by_method.items():
        ...
        row = {
            ...
            "abs_spearman": "" if rho is None else abs(rho),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="When --all-methods is used, evaluate this many methods at a time to bound memory.",
    )
    ...
    fieldnames = [
        ...
        "top1pct_recall_at_k",
        "top5pct_recall_at_k",
        "best_true_in_pred_top_k",
        "mean_true_in_pred_top_k",
    ]

    method_chunks = [methods]
    if args.all_methods and args.method_chunk_size > 0:
        method_chunks = [
            methods[i : i + args.method_chunk_size] for i in range(0, len(methods), args.method_chunk_size)
        ]

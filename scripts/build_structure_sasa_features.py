    ...
    with path.open(encoding="utf-8", errors="ignore") as fh:
        for line in fh:
            ...
            try:
                ...
                continue
            atoms.append({"resi": resi, "xyz": (x, y, z), "r": 1.90})
    return atoms


def align_atoms(
    atoms: list[dict[str, object]], target_positions: set[int], pdb_range: str
) -> list[dict[str, object]]:
    direct_overlap = sum(int(atom["resi"]) in target_positions for atom in atoms)
    match = re.fullmatch(r"\s*(\d+)\s*-\s*(\d+)\s*", pdb_range or "")
    if not match or not atoms:
        return atoms
    start = int(match.group(1))
    shifted_overlap = sum(
        start + int(atom["resi"]) - 1 in target_positions for atom in atoms
    )
...
def main() -> None:
    ...
    for meta in meta_rows:
        ...
        base = {
            "assay": assay,
            "protein_key": meta.get("protein_key", ""),
            "assay_type": meta.get("coarse_selection_type", ""),
            "has_local_structure_file": int(pdb_path is not None),
            "n_mutated_positions_with_sasa": 0,
            "n_variants_with_sasa": 0,
            "mean_mutation_site_sasa": "",
            "median_position_sasa": "",
            "sasa_exposed_fraction": "",
            "sasa_buried_fraction": "",
            "multi_mean_mutation_site_sasa": "",
            "multi_sasa_exposed_fraction": "",
            "multi_sasa_buried_fraction": "",
        }
        ...
        atoms = align_atoms(
            parse_atoms(pdb_path), target_positions, meta.get("pdb_range", "")
        )
        ...
        rec.update(
            {
                "n_mutated_positions_with_sasa": len(sasa),
                "n_variants_with_sasa": len(all_avg),
                "mean_mutation_site_sasa": safe_mean(all_avg),
                "median_position_sasa": vals[len(vals) // 2],
                "sasa_exposed_fraction": safe_mean(all_exp),
                "sasa_buried_fraction": safe_mean(all_bur),
                "multi_mean_mutation_site_sasa": safe_mean(multi_avg),
                "multi_sasa_exposed_fraction": safe_mean(multi_exp),
                "multi_sasa_buried_fraction": safe_mean(multi_bur),
            }
        )
        rows.append(rec)

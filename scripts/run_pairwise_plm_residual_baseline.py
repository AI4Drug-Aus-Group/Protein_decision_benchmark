...
def solve_linear(features: Sequence[Sequence[float]], targets: Sequence[float], ridge: float = 1e-6) -> Optional[List[float]]:
    if not features:
        return None
    p = len(features[0])
...
def metric_row(
    ...
    n_train_doubles: int,
    y_by_mutant: Dict[str, float],
    pred_by_mutant: Dict[str, float],
    pair_coverage: float,
) -> Dict[str, object]:
    ...
    return {
        "assay": assay,
        "seed": seed,
        "double_budget": budget,
        "model": model,
        "selection_policy": selection_policy,
        "n_train_doubles": n_train_doubles,
        "n_test_higher": len(common),
        "spearman_higher": "" if rho is None else rho,
        "abs_spearman_higher": "" if rho is None else abs(rho),
        "top1pct_recall_at_100": "" if recall is None else recall,
...
def process_assay(assay: str, seeds: Sequence[int], double_budgets: Sequence[int]) -> List[Dict[str, object]]:
    ...
    rows: List[Dict[str, object]] = [
        metric_row(assay, "all", 0, "additive_all_singles", "none", 0, y_higher, {m: additive_base[m] for m in higher}, 0.0)
    ]
    for method in PLM_METHODS:
    ...
    for seed in seeds:
        for budget in double_budgets:
            ...
            sampled_sets = [
                ("random_doubles", rng.sample(doubles, min(budget, len(doubles)))),
                ("informative_pair_coverage", informative_order[: min(budget, len(informative_order))]),
            ]
            for selection_policy, sampled in sampled_sets:
                ...
                rows.append(
                    metric_row(assay, seed, budget, "pairwise_residual_from_doubles", selection_policy, len(sampled), y_higher, pair_preds, coverage)
                )
                for method in PLM_METHODS:

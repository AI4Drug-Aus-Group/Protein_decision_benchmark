...
def process_assay(assay: str, seeds: Sequence[int], double_budgets: Sequence[int]) -> List[Dict[str, object]]:
    ...
    rows.append(
        {
            ...
            "seed": "all",
            "double_budget": 0,
            "model": "additive_all_singles",
            "selection_policy": "none",
            "n_train_doubles": 0,
            "n_test_higher": len(higher),
            "spearman_higher": "" if add_rho is None else add_rho,
            "abs_spearman_higher": "" if add_rho is None else abs(add_rho),
            "top1pct_recall_at_100": "" if add_recall is None else add_recall,
            "pair_coverage": 0.0,
        }
    )
    ...
    for seed in seeds:
        for budget in double_budgets:
            ...
            for selection_policy, sampled in [
                ("random_doubles", random_sampled),
                ("informative_pair_coverage", informed_sampled),
            ]:
                pair_residuals = {}
                for m in sampled:
                    ps = parts(m)
                ...
                rows.append(
                    {
                        ...
                        "model": "pairwise_residual_from_doubles",
                        "selection_policy": selection_policy,
                        "n_train_doubles": len(sampled),
                        "n_test_higher": len(higher),
                        ...
                        "pair_coverage": covered_pairs / total_pairs if total_pairs else 0.0,
                    }
                )
    return rows

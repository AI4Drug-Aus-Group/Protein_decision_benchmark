...
def main() -> None:
    ...
    with Path(args.metrics).open(newline="", encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            ...
            rec = {
                "best": float(row["best_observed_fitness"]),
                "best_percentile": float(row.get("best_observed_percentile", "nan")),
                "top1": float(row["top1pct_found"]),
                "n_observed": float(row["n_observed"]),
            }
        ...
        rows.append(
            {
                ...
                "round": round_idx,
                "n_runs": len(records),
                "mean_n_observed": mean(r["n_observed"] for r in records),
                "mean_best_observed_fitness": mean(r["best"] for r in records),
                "mean_best_observed_percentile": mean(r["best_percentile"] for r in records),
                "top1pct_found_rate": mean(r["top1"] for r in records),
            }
        )

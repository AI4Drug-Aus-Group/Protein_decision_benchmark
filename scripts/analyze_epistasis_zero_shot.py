...
def process_assay(assay: str, rows: Sequence[Dict[str, str]], methods: Sequence[str], min_n: int) -> List[Dict[str, object]]:
    ...
    out = [
        {
            "assay": assay,
            "method": "additive_prediction",
            "n": len(mutants),
            "spearman_double_score": "" if add_rho is None else add_rho,
            "abs_spearman_double_score": "" if add_rho is None else abs(add_rho),
            "gain_vs_additive_abs_spearman": 0.0,
            "spearman_epistasis_residual": "",
            "spearman_abs_epistasis_residual": "",
            "top100_high_abs_residual_recall": top_recall(high_abs, additive, 100),
            "top100_sign_epistasis_recall": top_recall(sign_epi, additive, 100),
        }
    ]
        ...
        out.append(
            {
                ...
                "method": method,
                "n": len(y),
                "spearman_double_score": "" if rho_score is None else rho_score,
                "abs_spearman_double_score": "" if rho_score is None else abs(rho_score),
                "gain_vs_additive_abs_spearman": gain,
                "spearman_epistasis_residual": "" if rho_res is None else rho_res,
                "spearman_abs_epistasis_residual": "" if rho_abs_res is None else rho_abs_res,
                "top100_high_abs_residual_recall": top_recall(high, pred, 100),
                "top100_sign_epistasis_recall": top_recall(sign, pred, 100),
            }
        )
    return out
...
def main() -> None:
    ...
    fieldnames = [
        ...
        "method",
        "n",
        "spearman_double_score",
        "abs_spearman_double_score",
        "gain_vs_additive_abs_spearman",
        "spearman_epistasis_residual",
        "spearman_abs_epistasis_residual",
        "top100_high_abs_residual_recall",
        "top100_sign_epistasis_recall",
    ]
    with out_path.open("w", newline="", encoding="utf-8") as fh:
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as pool:
            futures = [
                pool.submit(process_assay, assay, rows, methods, args.min_n)

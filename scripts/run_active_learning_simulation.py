...
POLICIES = [
    "random",
    "additive_lookup",
    "zscore_linear:ProSST-2048",
    "zscore_linear:ESM3",
    "additive_plus_zscore:ProSST-2048",
    "disagreement:additive_vs_ProSST-2048",
...
def diverse_top_batch(available: Sequence[str], score: Dict[str, float], batch: int, penalty: float = 0.02) -> List[str]:
    ...
    while ranked and len(chosen) < batch:
        best = max(
            ranked,
            key=lambda m: score.get(m, float("-inf")) - penalty * score_scale * len(mutation_sites(m) & chosen_sites),
        )
        chosen.append(best)
...
def simulate_assay_with_policies(
    assay: str,
    seeds: Sequence[int],
    rounds: int,
    batch: int,
...
def main() -> None:
    ...
    fieldnames = [
        ...
        "round",
        "n_observed",
        "best_observed_fitness",
        "best_observed_percentile",
        "top1pct_found",
        "pool_size",
        "retain_top_candidates",
    ]
    out = Path(args.out)
    with out.open("w", newline="", encoding="utf-8") as fh:
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as pool:
            futures = [
                pool.submit(simulate_assay_with_policies, assay, seeds, args.rounds, args.batch, args.max_candidates, not args.no_retain_top_candidates, policies)
                for assay in assays
            ]

    ...
    for path in inputs:
        ...
        with path.open(newline="", encoding="utf-8") as fh:
            for row in csv.DictReader(fh):
                ...
                groups[(row["budget"], row["regime"], row["model"], method_id)].append(
                    {
                        "best_percentile": pct if pct is not None else float("nan"),
                        "top1": top1 if top1 is not None else float("nan"),
                        "abs_rho": abs(rho) if rho is not None else float("nan"),
                    }
                )
        ...
        rows.append(
            {
                ...
                "n_rows": len(records),
                "mean_best_top100_percentile": mean(
                    r["best_percentile"] for r in records if r["best_percentile"] == r["best_percentile"]
                ),
                "mean_top1pct_recall_at_100": mean(r["top1"] for r in records if r["top1"] == r["top1"]),
                "mean_abs_spearman_all": mean(r["abs_rho"] for r in records if r["abs_rho"] == r["abs_rho"]),
            }
        )
    rows.sort(
        key=lambda r: (
            int(r["budget"]),
            r["regime"],
            -float(r["mean_best_top100_percentile"]),
            -float(r["mean_top1pct_recall_at_100"]),
        )
    )

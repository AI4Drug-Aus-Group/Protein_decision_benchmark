        ...
        out.append(
            {
                "assay": assay,
                "method": method,
                "subset": f"abs_residual_ge_q{quantile}",
                "threshold_abs_residual": threshold,
                "n": len(y),
                "spearman_score": "" if rho is None else rho,
                "abs_spearman_score": "" if rho is None else abs(rho),
                "spearman_residual": "" if rho_res is None else rho_res,
                "top1pct_recall_at_100": "" if top_recall(y, pred) is None else top_recall(y, pred),
            }
        )
    return out
...
def main() -> None:
    ...
    with out_path.open("w", newline="", encoding="utf-8") as fh:
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as pool:
            futures = [
                pool.submit(process_assay, assay, rows, args.quantile, args.min_n)

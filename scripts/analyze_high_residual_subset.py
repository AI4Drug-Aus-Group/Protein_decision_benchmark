...
def process_assay(assay: str, rows: Sequence[Dict[str, str]], quantile: float, min_n: int) -> List[Dict[str, object]]:
    abs_values = sorted((float(row['abs_epistasis_residual']) for row in rows))
    if len(abs_values) < min_n:
        return []
    threshold = abs_values[min(len(abs_values) - 1, max(0, int(math.floor(len(abs_values) * quantile))))]
    subset = [row for row in rows if float(row['abs_epistasis_residual']) >= threshold]
    if len(subset) < min_n:
        return []
    ...
    for method in METHODS:
        ...
        out.append({'assay': assay, 'method': method, 'subset': f'abs_residual_ge_q{quantile}', 'threshold_abs_residual': threshold, 'n': len(y), 'spearman_score': '' if rho is None else rho, 'abs_spearman_score': '' if rho is None else abs(rho), 'spearman_residual': '' if rho_res is None else rho_res, 'top1pct_recall_at_100': '' if top_recall(y, pred) is None else top_recall(y, pred)})
    return out
...
def main() -> None:
    ...
    parser.add_argument('--out', default=str(OUT_DIR / 'high_residual_subset_metrics.csv'))
    args = parser.parse_args()
    by_assay = load_residuals()
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    out_path = Path(args.out)
    fieldnames = ['assay', 'method', 'subset', 'threshold_abs_residual', 'n', 'spearman_score', 'abs_spearman_score', 'spearman_residual', 'top1pct_recall_at_100']
    with out_path.open('w', newline='', encoding='utf-8') as fh:
        writer = csv.DictWriter(fh, fieldnames=fieldnames)
        writer.writeheader()
        with ProcessPoolExecutor(max_workers=args.workers) as pool:
            futures = [pool.submit(process_assay, assay, rows, args.quantile, args.min_n) for assay, rows in by_assay.items()]

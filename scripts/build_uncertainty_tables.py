...
def main() -> None:
    ...
    grouped_ci(
        ...
        ["method"],
        "abs_spearman",
        OUT / "zero_shot_abs_spearman_ci.csv",
        {"method": top_zero_methods},
    )
    grouped_ci(
        ROOT / "results" / "single_to_multi" / "single_to_multi_representative_metrics.csv",
        ["order_bucket", "method"],
        "abs_spearman",
        OUT / "single_to_multi_abs_spearman_ci.csv",
        {
            "method": {
                "additive_all_singles",
                "ProSST-2048",
                "ESM3",
    ...
    grouped_ci(
        ...
        ["round", "policy"],
        "top1pct_found",
        OUT / "active_learning_top1pct_found_ci.csv",
        {"round": {"5"}},
    )
    grouped_ci(
        ROOT / "results" / "active_learning" / "active_learning_representative_metrics.csv",
        ["round", "policy"],
        "best_observed_percentile",
        OUT / "active_learning_best_percentile_ci.csv",
        {"round": {"5"}},
    )
    grouped_ci(
        ROOT / "results" / "multi_evolve_style" / "pairwise_plm_residual_higher_order_metrics.csv",
        ["model", "selection_policy", "double_budget"],
        "abs_spearman_higher",
        OUT / "pairwise_plm_abs_spearman_ci.csv",
    )

...
def metrics(y: np.ndarray, s: np.ndarray) -> dict[str, float]:
    ...
    top_true_10 = set(np.argsort(-y)[: max(1, math.ceil(0.10 * n))])
    ...
    return {
        "pearson": float(np.corrcoef(y, s)[0, 1]) if y_std > 0 and s_std > 0 else float("nan"),
        "rmse_z": float(np.sqrt(np.mean((ys - sscores) ** 2))),
        "mae_z": float(np.mean(np.abs(ys - sscores))),
        "top10_hit_rate_top1pct": hit10,
        "top20_hit_rate_top1pct": hit20,
        "top10_enrichment_top1pct": hit10 / random_rate if random_rate else float("nan"),
        "top20_enrichment_top1pct": hit20 / random_rate if random_rate else float("nan"),
        "top10_recall_top10pct": len(top_pred_10 & top_true_10) / len(top_true_10),
        "top20_recall_top10pct": len(top_pred_20 & top_true_10) / len(top_true_10),
    }
        ...
        summary.append(
            {
                "method": method,
                "n_assays": len(rows),
                "mean_pearson": mean([float(r["pearson"]) for r in rows if not math.isnan(float(r["pearson"]))]),
                "mean_rmse_z": mean([float(r["rmse_z"]) for r in rows if not math.isnan(float(r["rmse_z"]))]),
                "mean_mae_z": mean([float(r["mae_z"]) for r in rows if not math.isnan(float(r["mae_z"]))]),
                "mean_top10_enrichment_top1pct": mean([float(r["top10_enrichment_top1pct"]) for r in rows if not math.isnan(float(r["top10_enrichment_top1pct"]))]),
                "mean_top20_enrichment_top1pct": mean([float(r["top20_enrichment_top1pct"]) for r in rows if not math.isnan(float(r["top20_enrichment_top1pct"]))]),
                "mean_top10_hit_rate_top1pct": mean([float(r["top10_hit_rate_top1pct"]) for r in rows]),
                "mean_top20_hit_rate_top1pct": mean([float(r["top20_hit_rate_top1pct"]) for r in rows]),
            }
        )

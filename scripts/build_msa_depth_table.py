...
def msa_stats(path: Path) -> Dict[str, object]:
    ...
    return {
        "msa_file": path.name,
        "msa_n_sequences": n_seq,
        "msa_median_ungapped_length": sorted(lengths)[len(lengths) // 2] if lengths else 0,
        "msa_min_ungapped_length": min(lengths) if lengths else 0,
        "msa_max_ungapped_length": max(lengths) if lengths else 0,
    }

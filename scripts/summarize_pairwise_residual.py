...
def main() -> None:
    ...
    with metrics.open(newline="", encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            ...
            if rho is None:
                continue
            groups[(row["model"], row.get("selection_policy", "none"), row["double_budget"])].append(
                {
                    "rho": rho,
                    "recall": recall if recall is not None else float("nan"),
                    "coverage": coverage if coverage is not None else float("nan"),
                }
            )
    rows = []
        ...
        rows.append(
            {
                ...
                "n_rows": len(records),
                "mean_abs_spearman_higher": mean(r["rho"] for r in records),
                "mean_top1pct_recall_at_100": mean(r["recall"] for r in records if r["recall"] == r["recall"]),
                "mean_pair_coverage": mean(r["coverage"] for r in records if r["coverage"] == r["coverage"]),
            }
        )
